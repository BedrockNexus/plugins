...
export const githubWebhook = httpAction(async (ctx, request) => {
  ...
  try {
    claim = await ctx.runMutation(internal.github.webhooks.claimDelivery, {
      deliveryId,
      event,
  ...
  try {
    const result = await ctx.runMutation(internal.github.webhooks.processDelivery, {
    ...
    await ctx.runMutation(internal.github.webhooks.failDelivery, {
      webhookDeliveryId: claim.webhookDeliveryId,
      attemptNumber: claim.attemptNumber,
      error: message,
    });
