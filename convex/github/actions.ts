...
function normalizeRepository(data: {
  ...
  return {
    ...
    ...(data.language ? { primaryLanguage: data.language } : {}),
...
async function synchronizeRepositories(
  ...
  for (let index = 0; index < options.repositories.length; index += 75) {
    ...
    const result = await ctx.runMutation(internal.github.installations.syncRepositoryBatch, {
  ...
  do {
    const result: {
      ...
      isDone: boolean;
      updated: number;
    } = await ctx.runMutation(internal.github.installations.markMissingRepositoriesRemoved, {
      installationDocumentId: options.installationDocumentId,
      syncStartedAt: options.syncStartedAt,
  ...
  await ctx.runMutation(internal.github.installations.finishInstallationSync, {
    installationDocumentId: options.installationDocumentId,
...
export const createInstallUrl = action({
  ...
  handler: async (ctx) => {
    ...
    await ctx.runMutation(internal.github.installations.createInstallIntent, {
      tokenIdentifier,
...
export const completeInstallation = action({
  ...
  handler: async (ctx, args) => {
    ...
    const intent = await ctx.runMutation(internal.github.installations.beginInstallIntent, {
      tokenIdentifier,
      stateHash,
    });
    ...
    try {
      ...
      const claim = await ctx.runMutation(
        internal.github.installations.claimInstallationFromCallback,
        {
          tokenIdentifier,
          stateHash,
          installation,
      ...
      await ctx.runMutation(internal.github.installations.failInstallIntent, {
...
export const refreshInstallation = action({
  ...
  handler: async (ctx, args) => {
    const identity = await ctx.auth.getUserIdentity();
    const tokenIdentifier = requireIdentityTokenIdentifier(identity);
    const access = await ctx.runQuery(internal.github.installations.getInstallationAccess, {
      tokenIdentifier,
...
export const getRepositorySnapshot = action({
  ...
  handler: async (ctx, args): Promise<typeof repositorySnapshotValidator.type> => {
    ...
    } = await ctx.runQuery(internal.github.installations.getRepositoryAccess, {
      tokenIdentifier,
    ...
    try {
      ...
      return {
        ...
        ...(metadata.data.language ? { primaryLanguage: metadata.data.language } : {}),
